...
**Next (the queue) — REWRITTEN 2026-09-07, because every line of it was
stale.** It still listed phase 5 as upcoming, crypt waves C2–C4, "crypt
abilities are 0/217", AI v2 as unstarted and Go-getter superior as
unbuilt — all of them done, some of them for days. **This is the failure
that has cost this project more time than any bug** (five cut-list rows in
eight waves named a blocker that had since been built; the AI v2
"prerequisite" was a state object that already existed). *A queue is a
claim about the code as it was.* Re-read it against the tree before
believing it, and rewrite it when you finish something.

**Phases 1–7 are COMPLETE**: data pipeline, rules kernel, the whole V5
pool (library 444/444, crypt 217/217), the debug/play UI, AI v1 + batch
simulation, PeerJS multiplayer with lobby and room codes, and the deck
importer with 32 derived precons.

1. **PHASE 9 — PUBLISH.** Groundwork is done and verified
   (`docs/pages-design.md`): `base: "./"`, the deploy workflow, no
   root-absolute assets, the dev-only log sink correctly excluded, no SPA
   rewrite needed (a join link is a `#fragment`), a favicon, and the Dark
   Pack line on the first screen. **One item is left and it needs an image
   editor: the banner is 2160 KB of a 3.19 MB first paint — 68%.** Re-save
   at ~2200px as **WebP**, not JPEG: the alpha channel IS used (8.5% of
   pixels are partially transparent — measured, correcting an earlier note
   in this file that said otherwise). Deploying is the owner's act; nothing
   in this tree publishes.
2. **PHASE 8 — widen the pool beyond V5.** Owner has blessed it in
   principle. `data/vtes-raw.json` already holds all 4,149 KRCG cards, so
   widening is an edit to `config/v5-sets.json` plus
   `npm run cards:registry` — no network. Expect the count to jump from
   661 and every "X/444" tally in this file to need re-deriving. This is
   the months-long one.
3. **AI: weight-tweaking is exhausted, demonstrably** (see §5–§8 of
   `docs/richer-options-design.md` and `docs/ai-v2-design.md`). The search
   agent exists, is guarded, and is **not better yet**; what is unsolved is
   the VALUE FUNCTION, and after that, modelling a reply — which needs
   sampling an opponent's hidden cards and is always a guess, because deck
   lists are private.
...
2.5×, which is what a high-DPI screen wants. **CORRECTED 2026-09-07 — the
JPEG advice this paragraph used to give was WRONG.** It said the art is
"flat, dark, no transparency"; decoding the PNG with zlib says **the alpha
channel IS used** — 91.495% of pixels fully opaque, 8.502% PARTIAL — so a
JPEG would flatten the soft edge. Re-save at ~2200px as **WebP**, which
keeps the alpha and should land near 150–300 KB. It is 2160 KB of a
**3.19 MB first paint (68%)** and matters most on Pages, where every
visitor downloads it (`docs/pages-design.md` §3). Still no image tooling
here — and note `convert` on this machine is **Windows' FAT-to-NTFS volume
converter**, not ImageMagick, so do not run it on an image.
...
**A FIFTH PASS — "typing in the lobby chat starts the game, and the online
player is still in the lobby" (same doc, "The fifth pass").** The net layer
is **exonerated for the start** (a headless loopback repro has a guest send
a chat line and asserts neither `onStart` nor `onStarted` fires), there is
**no form in the document** and no global key handler — so Enter has no
default action to trigger a button with. The shell is DOM code and this
project has no jsdom, so rather than reproduce it I **closed every path by
which it could be true**, each a real defect: (1) **a host with a live room
must never start a PRIVATE game** — `start()` chose its path from
`isOnlineTable`, which reads SEAT KINDS, and those change as people arrive
and leave; if it ever read false with a room open, the host dealt a local
game with bots and nobody told the guests, which is symptom two exactly.
The room being open is the fact that decides. *The `buildTable` shape
again: a condition written against one instant, applied at another.*
(2) **a guest can start nothing** — `start()` returns immediately for a
guest, AND the lobby no longer **falls through to the host layout** when
`lobbyPeer.state` is null (genuinely null between `join` and the host's
first `lobby` message), where it had been drawing somebody else's table
with THIS client's default seats and a live Start button; that is the best
candidate for what was seen. (3) Enter in the chat `preventDefault`s and
`stopPropagation`s — no form exists *today*, and that box sits on a screen
whose other button deals a game.

**TWO REAL BUGS FOUND ON THE WAY, and they are the first pass's `onChanged`
bug one message type along: the chat repainted NEITHER end.**
`LobbyHost.say()` wrote to the store and sent to every guest but never
called its watchers, so the **host** saw nothing; `LobbyPeer`'s `chatLine`
branch called `addChat` without `emit()`, so a **guest** saw nothing. Only
`broadcast()` told the watchers, and neither of these broadcasts. `notify()`
is its own method now and `broadcast()` calls it, so the next path that
changes something without changing the table has one obvious thing to call.

**A SIXTH PASS — chat identity, emoji and being kicked (same doc, "The
sixth pass").** A player's **name colour lives on the PROFILE**
(`Profile.chatColor`), not in `settings.ts`, and the split is the point:
that file holds preferences that never leave the machine, while this is a
property of the PERSON — it travels with them and everyone at the table
sees them in it. Hence the same control behind the chat's gear and on the
profile page: **one value, two ways in**. It rides on `join` beside the
avatar and on `hello` (a **reconnect** arrives straight at the session and
never passes through the lobby), and **the host stamps it onto every line
it relays** — never copied from the sender's own message, for exactly the
reason `from` is not. `<input type="color">` is the wheel: the platform's
own picker, no dependency.

**THE COLOUR GATE DID NOT WORK, AND ITS OWN TEST FOUND IT.** A colour ends
up in a `style` attribute, so `colorProblem` insists on `#rrggbb` at one
gate on the way into the store — written as
`lines.push({ ...line, ...(ok ? { color } : {}) })`, which **keeps whatever
`line.color` held**: spreading an empty object removes nothing, so every
bad value went straight through (still escaped on the way out, so nothing
was injectable — but the gate was inert). The key is `delete`d now. **A
gate has to be asserted against the thing it exists to stop** — the
negative-space lesson, in the UI this time.

Also: an **emoji** grid beside Send, inserting at the caret (appending
would put a face added mid-sentence at the end); a **kick asks for a
reason** (`prompt`, where `null` is a cancelled kick and not an empty
reason) which travels in the `bye` the protocol already had and is quoted
back on a `removed` screen — without it, being kicked was
**indistinguishable from the connection dropping**; and a seat a bot takes
over is **relabelled, never renamed** (`SeatFace.label`,
`HostSession.botSeatNames`) because **a seat's name IS the engine's
identifier** and every option id, the command log and every saved game are
...
**WHICH VAMPIRE TO INFLUENCE — A MEASURED NEGATIVE RESULT, 2026-09-06
(`docs/richer-options-design.md` §7). DO NOT BUILD THIS AGAIN.** Influence
is the largest class of real choice (**39.6%**) and produced the one large
behavioural win so far, so the obvious next step was to prefer the vampire
that can play the cards actually in hand. **The signal is real and the
criterion is wrong.**

`LegalOption.transferToVampire.playableCards` counts them via
`modesPlayableBy` (a card's requirements live in the registry, which an
agent has no access to and should not — re-deriving them would be the
second model of the pool §5 refused). Measured: 2348 influence decisions
had more than one candidate, the candidates **differ on this in 28.1%** of
them, and weighting it at 0.5 **changes 9.0% of those decisions** — so it
is live, not an inert knob like `huntFutile`.

**It still does not help, and at strength it HURTS: weight 2 is −0.177 VP
on Hecata** (±0.110), negative-leaning on two more, neutral on the fourth.
Turning a knob up making things worse is the shape of a criterion that is
actively wrong. **A behavioural probe says why: the first vampire reaches
play at turn 6.46 instead of 5.08**, because preferring a *better* vampire
diverts counters from **finishing** a nearly-done one — and a
half-influenced vampire does nothing at all. That is exactly the mechanism
`influenceProgress` exploits. **Getting a body onto the table beats getting
the right body onto it**, by more than a whole turn.

Shipped: `influenceUnlocks` at **0** with a test pinning the zero, and
`playableCards` left on the option — **currently unconsumed by the
policy**, kept so the experiment is one flag away
(`--weights influenceUnlocks=2`) and the dead end is not re-derived, and
because the UI has an obvious use for it. A search agent would get this
right for free, since it would SEE the cards played rather than guess.

**THE PATTERN ACROSS THE LAST THREE ITEMS, all chosen off the decision
profile:** §5 `playCard` was a real gap with a measurable gain — but only
for 4 of 16 effect families, and the obvious full version was worse; §6
`takeAction` was mostly a mirage, and the fix that mattered was a bug in
the *blocking* decision found by reading the code next door; §7 influence
had real information and a wrong criterion. **Frequency tells you where to
look; it does not tell you there is anything there.** All three needed a
behavioural probe to explain a bench number, and in two of three the probe
overturned the reading the number invited.

**"THE POLICY HAS NO PLAN" — AND WHY WEIGHTS CANNOT GIVE IT ONE,
2026-09-06 (`docs/richer-options-design.md` §8). NOTHING SHIPPED, and the
negative result is the most useful thing in the AI thread.** Both places
where pressure should matter had a **CLIFF and no gradient**: a bleed that
would oust YOU is +50 to block and every other bleed scores the same at 25
pool as at 4; a bleed that would oust your PREY is +25 and every other one
scores the same whether they hold 4 or 25. The variance is real — the
blocker's own pool spans 7% at 0–3 / 22% at 4–7 / 35% at 13–20 / 20% at
21+, and 34% of bleed options face a prey at 0–3.

Two gradient terms were added and measured **separately** (the §5 lesson
applied before it could bite): **six exact zeros** across four decks. As in
§6 that is a signal, not a result, so the flip probe followed —
`blockPressure` flips **0 of 7361 decisions even at 5× strength**, and
`bleedPressure` flips **2**.

**THE LAW: a policy is an argmax over ONE option list, so a term that does
not VARY across that list cannot change the choice — and one that varies
only by enlarging an already-winning margin cannot either.** The prey's
pool is identical for every bleed option in a decision; my own pool is
identical for every blocker; and both bleeding (12) and blocking (6)
already beat passing (0.5) by a mile. A **drastically different blocking
policy** (decline small bleeds at high pool to stay unlocked), which really
does flip decisions, is *also* neutral on all four decks — two quite
different answers to "when should I block" produce statistically identical
games.

**That is the precise, measured reason item #5 is not reachable by
weights.** A plan is about arranging to face a different list later —
holding a vampire back to block next turn, building this turn so the next
bleeds for four — and a per-option score is evaluated *after* the list
exists. Both weights were **REMOVED rather than left at zero** (the
`influenceOut` precedent: a weight that provably does nothing is noise in a
table meant to be argued with), with the measurement recorded at both
cliffs so the missing gradient reads as a finding. Contrast
`influenceUnlocks`, kept at zero because it **does** flip 9% of decisions
and is a live experiment one flag away.

**WEIGHT-TWEAKING IS NOW EXHAUSTED DEMONSTRABLY, not by impression.**
Across §5–§8 the profile chose four places to look: `playCard` (11.7%) gave
a real gain but only for 4 of 16 effect families and the obvious full
version was worse; `takeAction` (12.6%) was mostly a mirage and the real
fix was a bug in the *blocking* decision found next door; influence (39.6%)
had real information and a wrong criterion, harmful at strength; pressure
was structurally invisible. What is left is **search (AI v2)**, whose whole
nature is to evaluate a move by the position it leads to rather than by a
price list — the one thing none of these entries could do.

**AI v2 — THE SEARCH AGENT IS BUILT AND MEASURED, 2026-09-06
(`docs/ai-v2-design.md`). The infrastructure is PROVEN; the lookahead is
NOT YET BETTER and loses by 1.08 VP at full strength.** `SearchAgent` is
opt-in and **not wired into the game's bots** — nothing the owner plays
against has changed.

**THE RECORDED BLOCKER WAS NOT THERE, for the sixth time in this project.**
`docs/ai-v1-design.md` §8 said an agent cannot legitimately have a state
because `PlayerView` carries no frame stack. True — and it assumed
`PlayerView` was the only masked object. **`redactFor(state, seat)` returns
a real `GameState`** with the whole frame stack and every unseeable card
blanked; it is what `viewFor` is defined in terms of and **what
`LocalTransport.stateFor` already sends a remote player**. So searching on
it is not a loophole: anything readable in it is readable by a person
playing that seat from another machine, it comes from THE masking rule
(no second set to get wrong), and the boundary test now covers it.
`Agent.decide` gained an optional 4th argument rather than a second
interface, so humans and AI keep one contract (principle 5).

**THE CRUX EXPERIMENT, run before designing anything: 7312 option
simulations on cloned masked states — 100.0% succeeded, 0 threw, 0 options
missing, 0.84 ms each.** Nothing needed determinizing for one ply: a
blanked card resolves to no handler and the enumerators already skip what
they cannot look up, so the totality this project has been fixing card by
card paid out in one line.

**THE MEASUREMENT (Hecata mirror, 120 games a row; search runs ~1 game/sec
against a policy's ~13, so margins are wider):** lookahead **0** →
**+0.150, inside ±0.294 — a dead heat, as the control must be**;
lookahead 0.3 → +0.008; lookahead **1 → −1.083 VP (±0.244)**. The control
is what makes it readable: with the lookahead off the agent reproduces the
policy exactly, so the damage is **the value function**, not the ordering,
the option cap or the simulation. **Its own control test caught a real flaw
in my instrument** — the search broke ties by candidate order where the
policy breaks them on its seeded stream, so `lookahead: 0` was not the
policy; the fix is also better behaviour (**the search must STRICTLY BEAT
the policy's own pick to override it**).

Two rescues were tried and neither moved it: **counting committed
influence** (a transfer is a pure loss to a one-ply look, and influence is
39.6% of choices) and **evaluating after RESOLUTION rather than after the
choice** (almost nothing in VTES pays off when it is chosen — announcing a
bleed transfers no pool and locks the actor). The second moved the result
by 0.03.

**WHAT IS ACTUALLY UNSOLVED IS THE VALUE FUNCTION**, and a search is only
as good as what it maximises: tempo is invisible (a locked and an unlocked
vampire score the same, so it cannot see that acting costs a block),
position is invisible (sitting between a strong predator and a weak prey is
worth a lot and appears nowhere), and one ply cannot see a trade — bleeding
into an open blocker and into a tapped-out table look identical, because
the reply is exactly what this agent may not model. Deeper search needs
**determinization**, and the first legitimate piece of it is that **a real
player knows their OWN decklist** (they built it) where `PlayerView` does
not say so.

**KNOWING YOUR OWN DECK — OWNER RULING 2026-09-06, BUILT
(`docs/ai-v2-design.md` §9): "The AI should know what cards they have in
their deck, their hand, and whatever is out on the table and is face-up."**
Two of the three were already true since phase 5; the third was a real gap
in the model, not a convenience — **masking could say what you may not SEE
and had no way to say what you already KNOW.** A player built their deck.

**`SeatState.deckList` is COMPOSITION, NEVER ORDER.** One entry per copy,
fixed at the deal; `redactFor` keeps it for its owner and **strips the key
entirely** for everyone else (deck lists are private in VTES — the whole
reason a search agent must GUESS at an opponent's hand rather than deduce
it). **The library stays face down even to its owner** (p. 14), so what
the owner gains is what is in the deck and never what is on top; the
arithmetic that follows — deck minus what I have drawn and played — is
what a player does in their head. Two tests pin the two halves, and the
key is genuinely ABSENT rather than present-and-undefined
(`exactOptionalPropertyTypes` insists, and anything walking the object
would otherwise still be told the seat has one).

**It bought the search CORRECTNESS, not strength.** A simulated
replacement draw used to hand the agent a blank card; `determinize()` now
fills its OWN library with the cards it knows are there — deck minus hand,
ash heap and cards in play — shuffled on the agent's seeded stream, **once
per decision** rather than once per option (or the comparison measures the
shuffle instead of the move). **Measured: it changed the result by nothing
at all**, to three decimals, because the value function counts hand SIZE
and not hand CONTENTS. **Three decimals of "no change" is also how
something looks when it never ran**, so `determinize` is public and
asserted directly: every card in its own library has a real name, every
name is really in its deck, no name appears more often than the deck holds
it, and **nobody else's library is touched**.
...
**Green baseline as of 2026-09-07: 178 test files, 1934 tests, typecheck and
...
**AI:** `ai-v2-design.md` (the search agent: why `redactFor` was the state an agent may legitimately have, the crux experiment, and the measured verdict that the value function — not the plumbing — is the open problem), `ai-v1-design.md` (the scoring policy, the batch harness, and the
